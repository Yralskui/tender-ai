...
export function isCharacteristicFieldName(text: string): boolean {
  ...
  if (/^(текстурирован|неопудрен|нестерильн|стерильн|цвет|длина|ширина|размер|назначение|манжета|толщин|плотност)/i.test(t)) {
    return true;
  }
  // «Материал» как поле характеристики, не «Материал для наложения гипсовой повязки»
  if (/^материал$/i.test(t)) return true;
  if (/^материал\s+(изготовлен|упаковк|внешн|внутренн|поверхност|основа|армирующ|плёнк|пленк)/i.test(t)) return true;
